...
public class ItemIntroManager : MonoBehaviour
{
    ...
    public void SetButtonActive(bool isHayStack, bool isTrap, bool isHeadphone, bool isPortal)
    {
        for (int i = 0; i < introList.Length; ++i) introList[i].gameObject.SetActive(false);
        if (isHayStack == false && isTrap == false && isHeadphone==false && isPortal == false)
        {
            helpButton.gameObject.SetActive(false);
            return;
        }
        int index = 0;
        bool[] buttonActives = { isHayStack, isTrap, isHeadphone, isPortal };
        ...
        for (int i = 0; i < buttonActives.Length; ++i)
        {
            if (buttonActives[i])
            {
                ...
                buttonList[i].GetComponent<RectTransform>().localPosition = new Vector3(0, -45 - buttonHeight * index, 0);
                index++;
            }
            else
            {
